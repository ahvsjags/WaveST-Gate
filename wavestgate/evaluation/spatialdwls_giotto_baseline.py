"""SpatialDWLS baseline runner through the Giotto/Seurat R package stack."""
...
REQUIRED_R_PACKAGES = ["Seurat", "Giotto", "GiottoClass", "GiottoUtils", "GiottoVisuals", "Rfast", "quadprog"]
...
def _check_giotto_ready() -> None:
    rscript = shutil.which("Rscript")
    if rscript is None:
        raise RuntimeError("SpatialDWLS/Seurat baseline requires Rscript.")
    checks = " && ".join(f"requireNamespace('{package}', quietly = TRUE)" for package in REQUIRED_R_PACKAGES)
    command = [rscript, "-e", f"quit(status = ifelse({checks}, 0, 1))"]
    if subprocess.run(command, check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode != 0:
        raise RuntimeError(f"SpatialDWLS/Seurat baseline requires R packages: {', '.join(REQUIRED_R_PACKAGES)}.")


def run_spatialdwls_giotto_baseline(
    ...
    """Run Giotto's runDWLSDeconv and evaluate proportions against a benchmark."""

    _check_giotto_ready()
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    batch, metadata, extra = load_prepared_dataset(prepared_path, device="cpu")
    genes_path = output_dir / "spatialdwls_giotto_genes.txt"
    cell_types_path = output_dir / "spatialdwls_giotto_cell_types.txt"
    _write_list(genes_path, list(metadata.gene_names))
    _write_list(cell_types_path, list(metadata.cell_types))

    r_script = Path(__file__).with_name("spatialdwls_giotto_baseline.R")
    command = [
    ...
    log_path = output_dir / "spatialdwls_giotto_run.log"
    log_path.write_text(completed.stdout, encoding="utf-8")
    if completed.returncode != 0:
        raise RuntimeError(f"SpatialDWLS/Seurat failed with exit code {completed.returncode}. See {log_path}")

    predictions_path = output_dir / "spatialdwls_giotto_proportions.csv"
    ...
    metrics_path = output_dir / "spatialdwls_giotto_metrics.csv"
    pd.DataFrame([row]).to_csv(metrics_path, index=False)
    r_manifest_path = output_dir / "spatialdwls_giotto_manifest.json"
    manifest = {
    ...
    (output_dir / "spatialdwls_giotto_python_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    return row


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Giotto SpatialDWLS on a WaveST-Gate benchmark.")
    ...
    row = run_spatialdwls_giotto_baseline(
